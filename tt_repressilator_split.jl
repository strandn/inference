...
function tt_repressilator()
    ...
    dom_full = [X10_dom, X20_dom, X30_dom, α1_dom, α2_dom, α3_dom, m_dom, η_dom]
    ...
    for cidx in 1:3
        ...
        for i in 1:d
            ...
            push!(dom, (avg - 5 * sd, avg + 5 * sd))
            println("Cluster $cidx")
            println(dom[i])
            push!(grid, collect(LinRange(dom[i]..., nbins + 1)))
        end
